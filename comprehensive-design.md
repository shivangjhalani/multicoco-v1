...
- `<img>`: Image start token (IMG_START_TOKEN)
- `</img>`: Image end token (IMG_END_TOKEN)
...
🤖 AI IMPLEMENTATION NOTE: Critical Data Processing Details from Coconut Codebase
...
3. **Position IDs**: Must be manually padded as `[0] * n_pad + list(range(len(input_ids)))` to maintain correct positioning
...
- **Standard Position Encoding**: Uses sequential position IDs for all tokens
...
class MultimodalCoconut(nn.Module):
        ...
        for pass_idx in range(max_n_latents):
            ...
            if filling_indices:
                ...
                for idx_pair in filling_indices:
                    ...
                    # Use hidden state from preceding position with correct offset
                    if token_idx > 0:
                        # CRITICAL: Use .detach() to break computational graph and prevent O(N²) memory growth
                        # This prevents autograd from back-propagating through all previous passes
    ...
    def generate(
        ...
        **kwargs
        ...
        3. **State Updates**: Updates attention masks and position IDs during generation
        ...
        # Step 3: Generate standard position IDs
        batch_size, seq_len = input_ids.shape
        position_ids = torch.arange(0, seq_len, dtype=torch.long, device=input_ids.device).unsqueeze(0).expand(batch_size, -1)
    ...
    def _coconut_autoregressive_generate(
        ...
        **kwargs
        ...
        for step in range(max_new_tokens):
            ...
            # Step 3: Handle dynamically generated latent tokens with proper multi-pass processing
            is_latent = (next_tokens == self.latent_token_id)
            
            if torch.any(is_latent):
                # CRITICAL: Process dynamically generated latent tokens with multi-pass logic
                # This preserves chained dependencies for BFS-like reasoning
                
                # First, append the latent tokens to the sequence
                next_token_embeds = self.language_model.get_input_embeddings()(next_tokens)
                
                # Update attention mask and position IDs for new tokens
                ...
                # Create temporary input_ids for the new tokens (needed for latent processing)
                temp_new_ids = next_tokens.unsqueeze(1)  # [batch_size, 1]
                
                # Process latent tokens with multi-pass logic (preserving dependencies)
                processed_embeds = self._process_dynamic_latents(
                    current_embeds=torch.cat([
                        # Reconstruct full sequence embeddings from KV cache context
                        self._reconstruct_embeddings_from_cache(past_key_values, current_attention.shape[1] - 1),
                        next_token_embeds.unsqueeze(1)
                    ], dim=1),
                    new_latent_positions=is_latent,
                    attention_mask=current_attention,
                    position_ids=current_pos_ids
                )
                
                # Use the processed embeddings for the next forward pass
                outputs = self.language_model(
                    inputs_embeds=processed_embeds[:, -1:, :],  # Only the processed new token
                    attention_mask=current_attention,
                ...
                # Standard token processing (non-latent)
                next_token_embeds = self.language_model.get_input_embeddings()(next_tokens)
                
                # Update attention mask and position IDs for new token
                new_attention = torch.ones(batch_size, 1, device=device)
                current_attention = torch.cat([current_attention, new_attention], dim=1)
                new_pos_ids = current_pos_ids[:, -1:] + 1
                current_pos_ids = torch.cat([current_pos_ids, new_pos_ids], dim=1)

                # Standard forward pass with KV cache
                outputs = self.language_model(
                    inputs_embeds=next_token_embeds.unsqueeze(1),
                    attention_mask=current_attention,
                    position_ids=new_pos_ids,
                    past_key_values=past_key_values,
                    output_hidden_states=True,
    ...
    def _reconstruct_embeddings_from_cache(self, past_key_values, seq_len):
        ...
        return {
            ...
            'scores': None,  # Could add generation scores if needed
            'past_key_values': past_key_values
        }
            original_ids = input_ids[batch_idx].tolist()
            generated_ids = generated_tokens[batch_idx]
            full_sequence = original_ids + generated_ids
            batch_outputs.append(torch.tensor(full_sequence, device=device))

        # Pad sequences to same length for batching
        if batch_outputs:
            max_length = max(len(seq) for seq in batch_outputs)
            padded_outputs = []
            for seq in batch_outputs:
            ...
            # Fallback: return original input if no generation occurred
            return input_ids
```
...
1. **Image Token Replacement**: `<image>` placeholder MUST be replaced with `IMG_START_TOKEN + IMG_CONTEXT_TOKEN * num_image_token * num_patches + IMG_END_TOKEN`
...
6. **Context Token ID**: MUST call `tokenizer.convert_tokens_to_ids(IMG_CONTEXT_TOKEN)` to get the correct token ID
...
def create_multimodal_prompt(self, question, choices, num_patches, image_present=True):
    ...
    if image_present:
        ...
        image_tokens = (
            self.config.IMG_START_TOKEN +
            self.config.IMG_CONTEXT_TOKEN * self.config.num_image_token * num_patches +
            self.config.IMG_END_TOKEN
        )
...
class VQAMultimodalProcessor:
    ...
    def process_sample(self, sample, stage=0):
        ...
        return {
            ...
            'position_ids': torch.arange(tokenized['input_ids'].shape[1]).unsqueeze(0)
        }
...
class MultimodalCoconutCollator:
    ...
    **Position IDs Handling**: The original Coconut approach of padding position_ids with zeros
    ...
    InternVL3 always generates position_ids dynamically as torch.arange(0, seq_len) after padding.
    ...
    def __call__(self, batch):
        ...
        position_ids = [item['position_ids'].squeeze() for item in batch]
...
│   ├── vqav2.yaml                  # VQAv2 specific config
│   ├── gqa.yaml                    # GQA specific config
│   └── textvqa.yaml                # TextVQA specific config
├── scripts/
│   ├── prepare_data.py             # Data preparation utilities
│   ├── evaluate_model.py           # Standalone evaluation
│   └── generate_samples.py         # Sample generation
├── data/
│   ├── aokvqa/                     # A-OKVQA dataset
│   ├── vqav2/                      # VQAv2 dataset
...
```yaml
# configs/aokvqa.yaml - A-OKVQA specific configuration
...
# Training specifics
training:
  batch_size: 6  # Reduced for A-OKVQA complexity
  num_epochs: 15
...
```yaml
# configs/vqav2.yaml - VQAv2 specific configuration
base_config: configs/base.yaml

# Dataset configuration
dataset:
  name: vqav2
  hf_dataset_id: HuggingFace/vqav2
  train_split: train
  val_split: validation
...
data:
  ...
  answer_format: "The answer is {answer}"
  image_data_path: ./data/vqav2/images
...
class MultimodalCoconutTrainer:
    ...
    def setup_model_and_data(self):
        ...
        self.image_processor = AutoImageProcessor.from_pretrained(self.config.model_id)

        # Use correct InternVL3 model loading pattern with config variables
        base_model = AutoModel.from_pretrained(
            self.config.model_id,
            torch_dtype=getattr(self.config, 'torch_dtype', torch.bfloat16 if self.config.bf16 else torch.float32),
            low_cpu_mem_usage=getattr(self.config, 'low_cpu_mem_usage', True),
            trust_remote_code=True,
            use_flash_attn=getattr(self.config, 'use_flash_attn', True)
        ).eval()
        ...
        config_params = {
            ...
            'ps_version': getattr(base_model.config, 'ps_version', 'v2'),
        ...
        # Add config parameters to self.config for use in model initialization
        ...
        if hasattr(base_model.config, 'vision_config'):
            ...
            self.config.num_image_token = int((image_size // patch_size) ** 2 * (downsample_ratio ** 2))
        else:
            self.config.num_image_token = 256  # Default fallback
        ...
        self.model = MultimodalCoconut(
            ...
            base_causallm=base_model.language_model,  # Keep reference to language model
        ...
        num_new_tokens = len(all_special_tokens)
        ...
        self.data_processor = AOKVQAMultimodalProcessor(
            self.tokenizer,
    ...
    def train_all_stages(self):
        ...
        for stage in range(self.config.max_latent_stage + 1):
            ...
            print(f"Latent replacement: {stage} reasoning steps → {stage * self.config.c_thought} latent tokens")
            ...
            if self.config.reset_optimizer or stage == 0:
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader, desc="Evaluating"):
                ...
                for i, output_ids in enumerate(outputs):
                    ...
                    predicted_choice = self.extract_choice_from_text(generated_text)
...
# Train on VQAv2 dataset
python run.py --config configs/vqav2.yaml --mode train
...
# Prepare VQAv2 data
python scripts/prepare_data.py --dataset vqav2 --output_dir ./data/vqav2
```

...
def main():
    ...
    for epoch in range(configs.resume, configs.num_epochs):
        ...
        if not configs.only_eval:
            ...
            # Scale learning rate by gradient accumulation steps and world size for effective batch size
            effective_batch_size = configs.batch_size_training * world_size * configs.gradient_accumulation_steps
            base_lr = configs.lr
            scaled_lr = base_lr * effective_batch_size / (configs.batch_size_training * world_size)
            
            # Update optimizer with scaled learning rate
            for param_group in optimizer.param_groups:
                param_group['lr'] = scaled_lr
            
            print(f"Rank {rank}: Base LR: {base_lr}, Scaled LR: {scaled_lr}, "
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if hasattr(configs, 'max_grad_norm') and configs.max_grad_norm > 0:
                    if (step + 1) % configs.gradient_accumulation_steps == 0 or step == len(train_dataloader) - 1:
                        torch.nn.utils.clip_grad_norm_(parallel_model.parameters(), configs.max_grad_norm)
